# Plateforme de migration d'ORACLE 11g vers POSTGRESQL 11 

Ce repository constitue une plateforme de migration de base de donnees Oracle 11g  vers Postgresql 11 TimescaleDB. Les 2 etapes de cette migration sont effectues respectivement avec la base de donnees Chinook et la base de donnees TPC-DS, DS signifie , Decision Support qui fournit un ensemble de scripts orientes benchmarks de performance pour prendre des decisions sur l'infrastructure de la base et sa normalisation SQL. 

![La_Plateforme](screenshots/migrate.png)
...
git clone https://github.com/financial_systems/ora2postgres.git
cd ci-cd
```
...
sudo yum install -y yum-utils \
  ...
  lvm2
```
Ensuite mettre en place le repository Docker.
```
sudo yum-config-manager \
    --add-repo \
    https://download.docker.com/linux/centos/docker-ce.repo
```
Installer la derniere version de Docker et ses packages client et containerd.io
```
...
## Plateforme de migration
### Soit lancement avec un docker-compose

Tapez la commande suivante pour installer et demarrer l'ensemble des containers de la plateforme de migration.

```
docker-compose up -d 
```
### Soit lancement par script Ansible 
```
ansible-playbook -i inventory install-migration
```
